...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    if (krypton_logo_ && back_icon_) {
      auto logo_width = gr_get_width(krypton_logo_.get());
      auto logo_height = gr_get_height(krypton_logo_.get());
      auto centered_x = ScreenWidth() / 2 - logo_width / 2;
      DrawSurface(krypton_logo_.get(), 0, 0, logo_width, logo_height, centered_x, y);
      y += logo_height;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  krypton_logo_ = LoadBitmap("logo_image");
...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int menu_rows = (ScreenHeight() - margin_height_*2 - gr_get_height(krypton_logo_.get()))
...
int ScreenRecoveryUI::SelectMenu(const Point& point) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(krypton_logo_.get());
      const static int logo_height = gr_get_height(krypton_logo_.get());
